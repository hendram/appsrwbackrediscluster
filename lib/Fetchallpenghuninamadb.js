...
export default async function fetchallpenghuninamadb(operatorname, nama, lowlimit = 0, highlimit = 10) {
  try {
    ...
    try {
      console.log('Checking if index penghuniIdx exists...');
      await client.ft.info('penghuniIdx');
      console.log('Index penghuniIdx exists');
    } catch (err) {
      console.error('Error fetching index info:', err.message);
    if (err.message.toLowerCase().includes('unknown index name')){
        console.log('Creating index penghuniIdx...');
        await client.ft.create('penghuniIdx', {
          '$.nama': {  
            type: client.ft.SchemaFieldTypes.TEXT,
            AS: 'nama',
            SORTABLE: true
          }
        }, {
